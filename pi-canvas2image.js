...
var PiCanvas2Image = function() {
    ...
    function saveFile(strData, fileName) {
        ...
        if (save_link.download === undefined) {
            document.location.href = strData; //IE直接在新一页中打开
            return;
        }
    ...
    var saveAsImage = function(canvas, fileName, width, height, type) {
        if ($support.canvas && $support.dataURL) {
            ...
            if (fileName === undefined) {
                var dt = new Date();
                fileName =
                    (Array(2).join(0) + dt.getYear()).slice(-2) +
                    (Array(2).join(0) + dt.getMonth()).slice(-2) +
                    (Array(2).join(0) + dt.getDay()).slice(-2) +
                    (Array(2).join(0) + dt.getHours()).slice(-2) +
                    (Array(2).join(0) + dt.getMinutes()).slice(-2) +
                    (Array(2).join(0) + dt.getSeconds()).slice(-2) +
                    '.' + type;
            }
            type = fixType(type);
            if (/bmp/.test(type)) {
